...
description: 日本語の技術文書、書籍原稿、調査記事を、主語、行為、対象、条件、因果、根拠を失わずに書くための規範。段落同士の論理関係、事実から著者の解釈へ進む論証、実例と仮想例の区別、過剰な抽象化と演出の抑制、LLM らしい空句と冗長の除去を扱う。日本語の記事、章、解説文を新規執筆または推敲するときに使用する。
---
...
- 一段落一トピックを、段落の独立性と取り違えない。各段落は、直前の段落にある問い、事実、用語、条件のいずれかを受け取り、説明、理由、証拠、対比、限定、結果のいずれかを加える。
- 段落の最初の文を読めば、その段落が何の話かわかるようにする。
- 段落の先頭では、前の段落との論理関係を内容で明示する。接続表現（「であれば」「実際」「しかし」「この例自体からも」）だけを足して、関係のない段落をつないだことにしない。
...
- 調査記事では、確認できた事実を並べて終わらない。具体的な事実または事例、比較によって分かる差、著者がそこから導く解釈、その解釈が成り立たない条件を追えるようにする。著者の解釈は根拠のない感想ではなく、複数の根拠を結ぶ論証として書く。
- 実在する導入事例、製品挙動、実験結果と、説明のために作った想定例を区別する。想定例はその旨を示し、何を説明するための例かを書き添える。
...
- 論証に役割を持たない固有名（後で参照しないファイル名、関数名、識別子）を出さない。ただし、根拠となる製品挙動、導入事例、論文、実装を特定する固有名は残す。一般名詞へ置き換えて根拠を失わせない。
...
- 読者が自力で一意に補え、主語、対象、条件、因果、根拠を失わない中間段階だけを省く。複数の解釈が可能になる省略はしない。
- 数文を一文に圧縮するのは、人物、行為、対象、条件、因果、証拠の役割が変わらない場合に限る。異なる事実を「状態」「範囲」「仕組み」のような上位語へ畳み込んで短くしない。
...
- 文脈を最短で読み手と共有できる文章にする。ただし、構造に名前を与えるのは、その構成要素を先に示し、以後その名前から中身を一意に復元できる場合に限る。命名によって説明を置き換えない。
...
- 節で扱っている対象を指す名詞句でもよい。
- 「状態」「範囲」「基盤」「文脈」「軸」のような上位語を見出しで使う場合は、前の節までに具体的な中身が定義されていることを確認する。未定義なら、誰が何をし、何が変わる節なのかを普通の動詞で書く。
...
- 調査で確認していない想定例を、公開事例や現行製品の実装であるかのように書かない。想定例が一般論の言い換えにすぎず、論証を進めない場合は削除する。
